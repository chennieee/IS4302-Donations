...
export default function CampaignCard({ c }) {
  ...
  const isMine =
    address && c?.organizer && address.toLowerCase() === c.organizer.toLowerCase()

  // Helper to convert amount to number (amounts are already in ETH)
  const toEth = (amount) => {
  ...
  const statusColour =
    status === 'completed' ? 'bg-green-500' : status === 'expired' ? 'bg-red-500' : 'bg-yellow-500'
  ...
  return (
    <>
      ...
      {/* Modal mount */}
      <AddProposalModal open={openProposal} onClose={() => setOpenProposal(false)} />
    </>
  )
}